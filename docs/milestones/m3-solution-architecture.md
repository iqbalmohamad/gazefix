...
C is B's field applied to a **sclera plate** — the eye with the source iris
removed and filled (A2, §8.8) — plus one rigidly translated iris layer, so the
iris exists exactly once. It is chosen over the strictly
...
(`GeometricCorrectionSettings.iris_layer = False`), which also switches the
plate off: B warps the real iris, and that is its whole mechanism. The
implementor builds B first (background field + mask + blend), gets the
pipeline of masks and the harness working, then adds the plate and the iris
layer together. The harness exposes the switch so
...
- **A** — its *full-quality* fill (`cv2.inpaint`) is the artifact-prone and
  costly part, and is build-dependent, which a deterministic suite cannot
  use. Rejected on that basis. Note that A2 (§8.8) does give C a fill of the
  source-iris region, but a cheap, deterministic scan-line one that sits
  **behind** the iris layer rather than having to reconstruct a
  presentation-quality patch.
...
| `sclera_plate` (variant C only, **A2**) | `roi_src` with the **iris hole** — the source iris disc grown by 1 px, intersected with `opening_mask` — replaced by sclera: for each scan-line along the eye axis `ex`, linearly interpolate between the nearest non-hole pixels inside the opening on either side; replicate the one available side if the hole reaches the opening edge on the other; and if a line has no non-hole interior pixel at all, use the median colour of the eye's non-hole interior pixels. If the eye has **no** non-hole interior pixel anywhere — reachable only at the corner of the §5.3 limits, where a large iris fills a small opening — the plate cannot be built and the frame is `FAILED (mask generation failed: no sclera to sample)`, never emitted with an invented colour. Scan-lines run along `ex`; on the axis-aligned ROI, pixel rows are an acceptable approximation (implementor's choice, as for `fillPoly`). Pixels outside the hole are copied unchanged, so nothing outside the opening is touched. Deterministic, ROI-local, ~35×35 px of work | the image the **background** field samples in variant C (§8.2), so the background carries no iris. Variant B does **not** use it — its whole mechanism is warping the real iris |
...
move at all; the vacated region behind a moving iris is filled with stretched
sclera — real sclera in variant B, plate sclera (synthetic inside the source
iris hole, §8.1) in variant C.
...
`iris_alpha` falls to 0 and that base shows through — after A2 that base is
plate sclera, so the clipped crescent is dropped rather than filled with
warped iris (§8.8, Q14); before A2 it degraded locally to B. That cut-off is
sharp by construction (§8.1): a
...
- **Exactly one iris is visible (A2).** No remnant of the iris at its
  original position and no double or smeared iris edge between the two
  positions. One bounded exception, by design rather than oversight: inside
  the `edge_px` ramp at the lid the composite still keeps `(1 − alpha)` of
  the canvas (§8.4), so where the source iris touched the lid a faint rim of
  it survives — at most 0.33 of one pixel ring at the default `edge_px`
  1.5 px, falling to 0.06 at the next. §15.2 bounds it; it must not become a
  visible dark band.
...
**strengthening, not a relaxation** — but the honest account is per row, not
a blanket claim. The frozen `0.6·|d|` floor governed exactly one row
(realistic **vertical** 15°); the realistic **horizontal** rows carried the
stricter `d ± 0.5 px`, and simulation confirms a correct implementation still
meets that (6.10 px against `d` 6.25, and 9.00 against 9.32), so those rows
are **not** relaxed. Only the vertical rows move to the ideal, because only
there is the frozen threshold unreachable. Every row is then expressed the
same way — within a fixed tolerance of the geometric ideal — and the v1.1
behaviour is rejected on all of them by 2.1–3.6 px, far outside the
tolerance.
...
there — the sweep across apertures 0.20–0.30 and iris ratios 0.32–0.45 shows
the deviation is anatomy-dependent by construction, reaching several pixels
where a large iris meets a tight aperture. Two consequences: the centroid row
is a fixture-bound regression check, not a general acceptance criterion; and
the **primary, threshold-free discriminator of this defect is the "exactly
one iris" structural assertion**, because for *horizontal* moves on a
barely-clipped eye the retained source iris overlaps the moved one and the
centroid is nearly blind to it (v1.1 reaches 0.98 of ideal in the sweep).

**Variant B's vertical floor is withdrawn, and why.** The frozen suite also
required variant B to reach `0.6·|d|` on the realistic vertical 15° case.
That is unreachable for the same geometric reason and by a wider margin —
simulation puts field-only B at 0.53–0.66 px against a 3.75/5.59 px floor,
because compressing the iris against the lid is precisely B's designed
weakness (§4.1). The floor is therefore withdrawn rather than restated: B
keeps a direction assertion, and the meaningful comparison is the
C-versus-B row, which the plate makes decisive.
...
6. the fill's degenerate fallbacks — one-sided, no interior sclera on a line,
   and no interior sclera anywhere (which must `FAIL` the frame rather than
   invent a colour) — are exercised deterministically;
7. the bounded lid-ramp remnant of §8.5 stays within its stated fraction.
...
build the opening mask, distance field, **in variant C the sclera plate
(§8.1, A2)**, and the warped layers (background remap — of the plate in C —
...
| mask generation raised or produced an empty/non-finite mask, or (*A2*) the sclera plate could not be built | FAILED | `mask generation failed: …` | 2 | input |
